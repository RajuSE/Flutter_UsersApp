...
void main() {
  group('Users Unit Test', () {
    ...
    test('returns number of users when http response is successful',
         ...
         List<User> ls=await service.getAllUsers();
         print("response list len: ${ls.length}");
          expect(ls.length>3, true);
        });

    test('return error message when http response is unsuccessful', () async {

      service.mockHttp = getMockClient(jsonFailedResponse);
      List<User> ls=await service.getAllUsersWrongAPI();
